...
import { buildStaffRegistrationJS } from "../utils/buildStaffRegistrationJS";

const WEBSITE_URL = "https://muscle-garage.entergym.in/";
const MIN_SPLASH_DURATION = 1500;
const SAFETY_TIMEOUT = 12000;
const FAB_HIDE_DELAY = 5000;
    ...
    >
      ...
      >
          ...
          <View style={StyleSheet.absoluteFill} pointerEvents="none">
            <View style={{ flex: 1, alignItems: "center", justifyContent: "center" }}>
              <Animated.Text
                style={{
                  color: labelColor,
                  fontSize: isBack ? 11 : isDone ? 17 : isRefreshingNow ? 10 : 18,
                  fontWeight: "800",
        ...
        >
          ...
          >
            {isRefreshingNow ? "LOADING" : isDone ? "DONE" : isBack ? "HOLD · BACK" : "REFRESH"}
          </Text>
        </View>
      </TouchableOpacity>
    </Animated.View>
...
export default function MainScreen() {
  ...
  const staffRegistered = useRef(false);
  const currentUrl     = useRef(WEBSITE_URL);

  const ownerFcmToken  = useOwnerNotifications();
  ...
  const resetIdleTimer = useCallback(() => {
    ...
    idleTimerRef.current = setTimeout(() => setUserVisible(false), FAB_HIDE_DELAY);
  }, []);
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onStartShouldSetPanResponderCapture: () => { resetIdleTimer(); return false; },
      onMoveShouldSetPanResponderCapture:  () => { resetIdleTimer(); return false; },
    }),
  ).current;
  ...
  const injectDeviceToken = useCallback(() => {
    ...
    const deviceName =
      `${ExpoDevice.brand ?? ""} ${ExpoDevice.modelName ?? ""}`.trim() || "Unknown";
    const js = `
    (function() {
      try {
  ...
  const injectStaffDevice = useCallback(() => {
    if (!ownerFcmToken || staffRegistered.current) return;
    ...
    webViewRef.current?.injectJavaScript(buildStaffRegistrationJS(ownerFcmToken));
    staffRegistered.current = true;
  }, [ownerFcmToken]);

  useEffect(() => {
  if (ownerFcmToken) {
    staffRegistered.current = false;  // ← allow re-registration with new token
    injectStaffDevice();
  }
}, [ownerFcmToken, injectStaffDevice]);
  ...
  useEffect(() => {
    ...
    return () => { if (bgLocationRef.current) clearInterval(bgLocationRef.current); };
  }, [startLocationPolling]);
  ...
  const dismissLoading = useCallback(() => {
    const delay = Math.max(0, MIN_SPLASH_DURATION - (Date.now() - loadStartTime.current));
    setTimeout(() => {
  ...
  const handleLoadProgress = useCallback(
    ({ nativeEvent }: { nativeEvent: { progress: number } }) => {
      ...
      if (nativeEvent.progress >= 0.9) { clearSafetyTimer(); dismissLoading(); }
    },
  ...
  const handleLoadEnd = useCallback(() => {
    ...
    injectDeviceToken();
    injectStaffDevice();
  }, [clearSafetyTimer, dismissLoading, injectLocation, injectDeviceToken, injectStaffDevice]);
  ...
  const handleNavigationStateChange = useCallback(
    (navState: { url: string; canGoBack: boolean }) => {
      ...
      if (navState.url.includes("/login/") || navState.url.includes("/logout/")) {
        staffRegistered.current = false;
      }
    },
    [],
  );
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      {loading && (
        <Animated.View
          ...
          pointerEvents="none"
        >
          <OverlayScreen title={isRefreshing ? "Refreshing..." : "Loading..."} />
        </Animated.View>
      )}
...
const styles = StyleSheet.create({
  ...
  tagText: { color: "#ff4d00", fontSize: 10, fontWeight: "800", letterSpacing: 4 },
  overlayTitle: {
    color: "#fff",
    fontSize: 24,
    fontWeight: "800",
  ...
  bottomLabel: { color: "#333", fontSize: 9, letterSpacing: 3, fontWeight: "600" },
  barsContainer: {
    position: "absolute",
    overflow: "hidden",
  ...
  torusPillText:   { color: "#3a3a3a", fontSize: 8, fontWeight: "800", letterSpacing: 1.5 },
